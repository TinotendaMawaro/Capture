...
-- Ministry Registration and Management System
...
-- Function to generate unique zone ID
-- Format: R + region_code + zone_number (e.g., R01001)
-- Uses row-level locking to ensure uniqueness
CREATE OR REPLACE FUNCTION generate_zone_id(p_region_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_region RECORD;
...
BEGIN
  ...
  -- Format: R + region_code (without R prefix) + padded zone number
  v_zone_number := LPAD(v_zone_count::TEXT, 3, '0');
...
-- Function to generate unique pastor ID
-- Format: R + zone_code + P + sequential_number (e.g., R01001P1)
-- Uses row-level locking to ensure uniqueness
CREATE OR REPLACE FUNCTION generate_pastor_id(p_zone_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_zone RECORD;
...
BEGIN
  -- Get zone info
  SELECT full_code, region_id INTO v_zone
  FROM zones
  WHERE id = p_zone_id;
  ...
  SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'P([0-9]+)$')::INTEGER), 0) + 1 INTO v_pastor_count
  FROM pastors
  WHERE zone_id = p_zone_id;

  -- Format: zone_full_code + P + padded number
  v_pastor_number := LPAD(v_pastor_count::TEXT, 2, '0');
...
-- Function to generate unique deacon ID
-- Format: R + zone_code + D + sequential_number (e.g., R01001D1)
-- Uses row-level locking to ensure uniqueness
CREATE OR REPLACE FUNCTION generate_deacon_id(p_zone_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_zone RECORD;
...
BEGIN
  ...
  SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'D([0-9]+)$')::INTEGER), 0) + 1 INTO v_deacon_count
  FROM deacons
  WHERE zone_id = p_zone_id;

  -- Format: zone_full_code + D + padded number
  v_deacon_number := LPAD(v_deacon_count::TEXT, 2, '0');
...
-- Format: R + zone_code + DEP + sequential_number (e.g., R01001DEP01)
-- Uses row-level locking to ensure uniqueness
CREATE OR REPLACE FUNCTION generate_department_id(p_zone_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_zone RECORD;
  v_dep_count INTEGER;
...
BEGIN
  ...
  SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'DEP([0-9]+)$')::INTEGER), 0) + 1 INTO v_dep_count
  FROM departments
  WHERE zone_id = p_zone_id;

  -- Format: zone_full_code + DEP + padded number
...
-- Format: R + zone_code + M + sequential_number (e.g., R01001M001)
-- Uses row-level locking to ensure uniqueness
CREATE OR REPLACE FUNCTION generate_member_id(p_zone_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_zone RECORD;
...
BEGIN
  ...
  LOCK TABLE church_members IN ACCESS EXCLUSIVE MODE;
  
  SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'M([0-9]+)$')::INTEGER), 0) + 1 INTO v_member_count
  FROM church_members
  WHERE zone_id = p_zone_id;
...
-- Function to get next available number for any entity type
-- Useful for checking what the next ID would be without creating
CREATE OR REPLACE FUNCTION get_next_id_number(
  p_entity_type TEXT,
  p_zone_id UUID
)
RETURNS INTEGER AS $$
...
BEGIN
  CASE p_entity_type
    WHEN 'pastor' THEN
      SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'P([0-9]+)$')::INTEGER), 0) + 1
      INTO v_count
      FROM pastors
      WHERE zone_id = p_zone_id;
      
    WHEN 'deacon' THEN
      SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'D([0-9]+)$')::INTEGER), 0) + 1
      INTO v_count
      FROM deacons
      WHERE zone_id = p_zone_id;
      
    WHEN 'department' THEN
      SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'DEP([0-9]+)$')::INTEGER), 0) + 1
      INTO v_count
      FROM departments
      WHERE zone_id = p_zone_id;
      
    WHEN 'member' THEN
      SELECT COALESCE(MAX(SUBSTRING(full_code FROM 'M([0-9]+)$')::INTEGER), 0) + 1
      INTO v_count
      FROM church_members
      WHERE zone_id = p_zone_id;
...
CREATE OR REPLACE FUNCTION is_valid_id_format(p_full_code TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  -- Zone format: R\d{5}
  IF p_full_code ~ '^R\d{5}$' THEN
    RETURN TRUE;
  END IF;
  
  -- Pastor format: R\d{5}P\d{2}
  IF p_full_code ~ '^R\d{5}P\d{2}$' THEN
    RETURN TRUE;
  END IF;
  
  -- Deacon format: R\d{5}D\d{2}
  IF p_full_code ~ '^R\d{5}D\d{2}$' THEN
    RETURN TRUE;
  END IF;
  
  -- Department format: R\d{5}DEP\d{2}
  IF p_full_code ~ '^R\d{5}DEP\d{2}$' THEN
    RETURN TRUE;
  END IF;
  
  -- Member format: R\d{5}M\d{3}
  IF p_full_code ~ '^R\d{5}M\d{3}$' THEN
    RETURN TRUE;
  END IF;
...
CREATE TRIGGER regions_update_timestamp BEFORE UPDATE ON regions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER zones_update_timestamp BEFORE UPDATE ON zones
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER departments_update_timestamp BEFORE UPDATE ON departments
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER pastors_update_timestamp BEFORE UPDATE ON pastors
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER deacons_update_timestamp BEFORE UPDATE ON deacons
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER church_members_update_timestamp BEFORE UPDATE ON church_members
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER users_update_timestamp BEFORE UPDATE ON users
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER qr_codes_update_timestamp BEFORE UPDATE ON qr_codes
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
